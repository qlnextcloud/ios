...
@interface CCScanViewController ()<AVCaptureMetadataOutputObjectsDelegate,CALayerDelegate>
...
- (void)viewDidLoad {
    ...
    self.annulla.tintColor = [UIColor whiteColor];
    [self.annulla setTitle:NSLocalizedString(@"_cancel_", nil) forState:UIControlStateNormal];
    [self.alignMessage setText:NSLocalizedString(@"_please_align_QR_code_", nil)];
    // 设置扫描二维码
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if ([metadataObjects count ] > 0 ) {
        ...
        if(metadataObject.stringValue == nil){
            ...
            NSLog(@"metadataObject.stringValue !== nil");
    ...
    //将扫描后的结果返回到上一层页面
    if (self.resultBlock && stringValue) {
...
- (IBAction)handleAnnulla:(UIButton *)sender {
    [self dismissViewControllerAnimated:YES completion:nil];
}
